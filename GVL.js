...
export class GVL extends Cloneable {
    ...
    */
    async fetchJson(url) {
        const cacheKey = `GVL_JSON_${this.isLatest ? 'latest' : this.vendorListVersion}`;
        ...
        if (cachedData && cachedExpiry && now < parseInt(cachedExpiry, 10)) {
            ...
            // Fetch from the given URL and store in sessionStorage
            return fetch(url)
                .then(response => {
                    if (!response.ok) {
                        throw new GVLError(`Failed to load GVL JSON from ${url}`);
                    }
                ...
                .catch(error => {
                    throw new GVLError(`Error fetching GVL JSON: ${error.message}`);
                });
        }
    }
    /**
